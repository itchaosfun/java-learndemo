...
class SuffixArray
{
    ...
    {
           // radix book.adt.learn.sort the new character trios
    ...
    // stably book.adt.learn.sort in[0..n-1] with indices into s that has keys in 0..K
    // into out[0..n-1]; book.adt.learn.sort is relative to offset into s
    // uses counting radix book.adt.learn.sort
    ...
    { 
        ...
        for( int i = 0; i < n; i++ )
            out[ count[ s[ in[ i ] + offset ] ]++ ] = in[ i ];      // book.adt.learn.sort
    } 
